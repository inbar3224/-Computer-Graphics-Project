...
void to_json(nlohmann::json &jsonObject,
             const Matrix<ScalarType, rows, cols> &matrix) {
...
void from_json(const nlohmann::json &jsonObject,
               Matrix<ScalarType, rows, cols> &matrix) {
    nlohmann::json jsonArray;
    ...
    if ((rows >= 0 && int(providedRows) != rows) ||
        (cols >= 0 && int(providedCols) != cols)) {
        std::cerr << "Expected matrix of size " << rows << "x" << cols
                  << ", received matrix of size " << providedRows << "x"
