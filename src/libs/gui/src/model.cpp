...
namespace crl {
	namespace gui {
		...
		void Model::draw(const Shader& shader, const V3D& color, float alpha) const {
			shader.use();
		...
		void calculateFaceNormals(const Eigen::MatrixXd& V, const Eigen::MatrixXi& F, Eigen::MatrixXd& FN) {
			FN.resize(F.rows(), 3);
			...
			for (uint i = 0; i < materials.size(); ++i) {
				...
				if (!mat.diffuse_texname.empty())
					material_textures[i][Mesh::DIFFUSE].push_back({
						textureFromFile(mat.diffuse_texname.c_str(), directory), mat.diffuse_texname.c_str()
						});
