...
namespace crl {
	namespace gui {
		...
		public:
			Fancy3DApp(const char* title = "Shadows demo", std::string iconPath = DATA_FOLDER "/crl_icon_grey.png") : Application(title, iconPath) {
			...
			Shader silhouetteShader = Shader(SHADER_FOLDER "/silhouette2.vert", SHADER_FOLDER "/silhouette1.geom", SHADER_FOLDER "/silhouette.frag", "silhouetteShader");
		private:
			int PCF_mode = 0;
			int PCF_samples_num = 25;
			float shadow_spread = 0.002f;
			float light_size = 1;
		};
	}  // namespace gui
}  // namespace crl